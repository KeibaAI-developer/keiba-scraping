...
馬券発売後のオッズ取得関数（`scrape_odds_from_netkeiba`, `scrape_odds_from_jra`）は共通のカラム構成（ODDS_COLUMNSの6カラム）を持つ。
馬券発売前の予想オッズ取得関数（`scrape_yoso_odds_from_netkeiba`）は独自のカラム構成（YOSO_ODDS_COLUMNSの3カラム）を持つ。

#### オッズカラム (ODDS_COLUMNS)

- **カラム**: 馬番、単勝オッズ、単勝人気、複勝最小オッズ、複勝最大オッズ、複勝人気

#### 予想オッズカラム (YOSO_ODDS_COLUMNS)
...
netkeibaのオッズAPIから単勝・複勝オッズを取得する。

- **引数**: `race_id`, `config`
- **戻り値**: オッズ情報のDataFrame（ODDS_COLUMNSのカラム）
- **スクレイピング対象URL**: `https://race.netkeiba.com/api/api_get_jra_odds.html?race_id={race_id}&type=1`
- **馬券発売前**: オッズデータが存在しないため、空のDataFrame（0行、カラムのみ）を返す
- **馬券発売後**: JSON APIからオッズを取得
  - 出走取消馬が含まれる場合がある（オッズが"---"等）。出走取消馬の単勝オッズ・複勝オッズはNaN
- **KeibaAIとの差異**:
  - KeibaAIでは出馬表ページからSeleniumでオッズを取得していたが、オッズ専用APIからrequestsで取得するよう変更
  - カラムを統一（馬番、単勝オッズ、単勝人気、複勝最小オッズ、複勝最大オッズ、複勝人気）
  - 馬名カラムはODDS_COLUMNSから削除（馬番で識別可能なため）
  - KeibaAIでは「オッズ種別」カラムで予想/確定を区別していたが、廃止（馬券発売前は0行で判別可能）
  - Seleniumを使用しない（requestsに変更）

##### `scrape_yoso_odds_from_netkeiba(race_id: str, config: ScrapingConfig | None = None) -> pd.DataFrame`

netkeibaの出馬表ページから予想オッズを取得する（馬券発売前用）。Seleniumを使用してJavaScriptで生成されるオッズを取得する。

- **引数**: `race_id`, `config`
- **戻り値**: 予想オッズ情報のDataFrame（YOSO_ODDS_COLUMNSのカラム）
- **スクレイピング対象URL**: `https://race.netkeiba.com/race/shutuba.html?race_id={race_id}`
- **取得項目**:
  - 馬番: 枠順確定前は空のためNaN
  - 馬名: 馬を識別するための馬名
  - 予想単勝オッズ: 予想オッズ（馬券発売前のみ表示される）
- **注意事項**:
  - Seleniumを使用するためChromeDriverが必要
  - 枠順確定前は馬番がNaNになるが、馬名で識別可能

##### `scrape_odds_from_jra(race_id: str, config: ScrapingConfig | None = None) -> pd.DataFrame`

JRAから単勝・複勝オッズをスクレイピングする（async関数）。Playwrightでページを操作してオッズを取得する。

- **引数**: `race_id`, `config`
- **戻り値**: オッズ情報のDataFrame（ODDS_COLUMNSのカラム）
- **スクレイピングロジック**: KeibaAIの`_scrape_odds_from_jra`と同一ロジック
  - Playwright（Chromium）でJRA公式サイトにアクセスし、オッズページに遷移
  - `pd.read_html`でテーブルを取得し、馬番・単勝・複勝を抽出
  - 複勝は`-`区切りで複勝最小オッズ・複勝最大オッズに分割
  - 7頭立て以下は「複勝（2着払い）」、8頭立て以上は「複勝（3着払い）」
...
- `build_odds_url(race_id, config)`: netkeibaオッズページURL（`utils.py`に追加）
...
- `build_odds_url(race_id, config)`: netkeibaオッズページURL
...
- Selenium（ChromeDriver）は馬柱、レーススケジュール（`race_schedule.py`）の取得に使用
- Playwright（Chromium）はJRAオッズの取得に使用
- netkeibaオッズはrequestsで取得（Selenium不使用）
