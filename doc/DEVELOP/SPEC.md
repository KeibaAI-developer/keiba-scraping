...
ページごとにスクレイパークラスを設けて、コンストラクタでHTTPリクエストとBeautifulSoupの生成を行い、各メソッドでテーブルを取得する。HTMLからテーブルを抽出・整形する処理は各モジュール（`race_info.py`、`result.py`等）にパブリック関数として実装する。
...
スクレイパークラス (entry_page.py, result_page.py 等)
  ...
  │ get_result(): result.scrape_result(html, ...) を呼び出し
  │ ...
  ↓
各モジュール (race_info.py, result.py 等)
  │ soup/htmlを受け取り、テーブルを抽出・整形して返す
  ↓
pd.DataFrame
```
...
| `ResultPageScraper` | `result_page.py` | 結果ページ | `get_race_info()`, `get_result()`, `get_corner()`, `get_payoff()`, `get_lap_time()` |
| `HorsePageScraper` | `horse_page.py` | 馬情報ページ | `get_past_performances()` |

各モジュールのパブリック関数はスクレイパークラスからだけでなく、直接soupを渡して呼び出すことも可能。これにより単体テストではスクレイパークラスを経由せずにテストできる。
...
│   ├── race_info.py       # レース情報の抽出・整形
│   ├── result.py          # レース結果の抽出・整形
│   ├── entry.py           # 出馬表の抽出・整形
│   ├── horse.py           # 馬情報・馬柱の抽出・整形
│   ├── odds.py            # オッズの抽出・整形
│   ├── schedule.py        # レーススケジュール・カレンダー
│   ├── entry_page.py      # 出馬表ページスクレイパークラス
│   ├── result_page.py     # 結果ページスクレイパークラス
...
| レース名 | str | レース名 |
| 発走時刻 | datetime | 発走時刻（日付と時刻の組み合わせ） |
...
- `_build_race_info_dict(race_id, race_info_list)`: リストからRACE_INFO_COLUMNSに対応するdictに変換。コース生値を左右・コース・内外に分割し、数値文字列をint型に変換
...
##### `scrape_result(soup: BeautifulSoup, html_text: str, config: ScrapingConfig | None = None) -> pd.DataFrame`

BeautifulSoupからレース結果の表を抽出する。

- **引数**: `soup`, `html_text`, `config`
- **戻り値**: レース結果のDataFrame
...
##### `scrape_corner(soup: BeautifulSoup, config: ScrapingConfig | None = None) -> pd.DataFrame`

BeautifulSoupからコーナー通過順位を抽出する。

- **引数**: `soup`, `config`
- **戻り値**: コーナー通過順位のDataFrame（直線レースの場合は空のDataFrame）
- **カラム**: コーナー（1〜4コーナー）、通過順

##### `scrape_payoff(html_text: str, config: ScrapingConfig | None = None) -> pd.DataFrame`

HTMLから払い戻し情報を抽出する（KeibaAIでの`pay_back`に対応）。
...
##### `scrape_lap_time(html_text: str, race_type: str = "平地", config: ScrapingConfig | None = None) -> pd.DataFrame`

HTMLからラップタイムを抽出する（KeibaAIでの`rap`に対応、名前を修正）。

- **引数**: `html_text`, `race_type`, `config`
...
- `get_race_info(race_id)` → `race_info.scrape_race_info()` を呼び出し
...
##### `scrape_entry(soup: BeautifulSoup, html_text: str, config: ScrapingConfig | None = None) -> pd.DataFrame`

BeautifulSoupから出馬表を抽出する（KeibaAIでの`shutuba`に対応）。
...
|---|---|---|
| `scrape_shutuba_page` | `EntryPageScraper` | スクレイパークラス化、馬柱取得を含まない、date_id不使用 |
| `scrape_result_page` | `ResultPageScraper` | スクレイパークラス化、date_id不使用 |
