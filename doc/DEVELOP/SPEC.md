...
### 5.9. `schedule.py` — レーススケジュール・カレンダー
...
| `scrape_horse_info_page` | `HorseInfoScraper` | スクレイパークラス化、param不使用、コンストラクタで最大ページ数を自動取得、horse_id指定による1行取得に対応、所属値変換（東→美浦等）、総賞金・生年を追加（父ID・母ID・母父IDは馬一覧テーブルから取得不可のため非対応） |
