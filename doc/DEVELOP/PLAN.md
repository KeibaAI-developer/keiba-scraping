...
| PR-6 | 馬情報・馬柱スクレイピングの実装 | PR-2 | 大 | 未着手 |
| PR-7 | オッズスクレイピングの実装 | PR-2 | 中 | 未着手 |
| PR-8 | レーススケジュール・カレンダーの実装 | PR-2 | 中 | 未着手 |
| PR-9 | 馬情報ページスクレイパークラスの実装 | PR-6 | 中 | 未着手 |
...
## PR-9: 馬情報ページスクレイパークラスの実装

### 概要

馬情報ページのスクレイパークラスを実装する。
`EntryPageScraper`と`ResultPageScraper`はPR-4およびPR-5で実装済みのため、
本PRでは`HorsePageScraper`のみを実装する。

### 作業内容

1. `scraping/horse_page.py`の実装
...
| テストファイル | テスト対象 | ケース |
|---|---|---|
| `test/unit/page/test_horse_page_scraper.py` | `HorsePageScraper` | コンストラクタ、get_past_performances |
...
PR-4  PR-5 馬情報ページ
結果   出馬表  スクレイパー
(※ResultPageScraper   (※HorsePageScraper)
 含む)  (※EntryPageScraper
         含む)
  │    │     │
  ├────┘     │
