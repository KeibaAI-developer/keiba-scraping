"""ResultPageScraper.get_quinella_payoff()の単体テスト
...
from scraping.config import QUINELLA_PAYOFF_COLUMNS
...
def test_columns_match(race_id: str) -> None:
    """カラム構成がQUINELLA_PAYOFF_COLUMNSと一致すること"""
...
def test_first_slot_is_not_nan(race_id: str) -> None:
    ...
    assert pd.notna(df["馬連組番_1_1"].iloc[0])
    assert pd.notna(df["馬連組番_1_2"].iloc[0])
    assert pd.notna(df["馬連人気_1"].iloc[0])
...
def test_derby_2025_values() -> None:
    """ダービー2025の馬連払い戻しが正しいこと"""
    scraper = create_scraper_from_fixture("202505021211")
    df = scraper.get_quinella_payoff()

    assert df["馬連払戻金_1"].iloc[0] == 560
    assert df["馬連組番_1_1"].iloc[0] == 13
    assert df["馬連組番_1_2"].iloc[0] == 17
    assert df["馬連人気_1"].iloc[0] == 1
    assert pd.isna(df["馬連払戻金_2"].iloc[0])


def test_oaks_2010_dead_heat_first() -> None:
    """オークス2010（1着同着2頭）の馬連払い戻しが正しいこと"""
    scraper = create_scraper_from_fixture("201005030211")
    df = scraper.get_quinella_payoff()

    assert df["馬連払戻金_1"].iloc[0] == 1870
    assert df["馬連組番_1_1"].iloc[0] == 17
    assert df["馬連組番_1_2"].iloc[0] == 18
    assert df["馬連人気_1"].iloc[0] == 7
    assert pd.isna(df["馬連払戻金_2"].iloc[0])


def test_three_way_dead_heat_third() -> None:
    """3着同着3頭レースの馬連払い戻しが正しいこと"""
    scraper = create_scraper_from_fixture("202009050712")
    df = scraper.get_quinella_payoff()

    assert df["馬連払戻金_1"].iloc[0] == 2110
    assert df["馬連組番_1_1"].iloc[0] == 8
    assert df["馬連組番_1_2"].iloc[0] == 18
    assert df["馬連人気_1"].iloc[0] == 7
    assert pd.isna(df["馬連払戻金_2"].iloc[0])
