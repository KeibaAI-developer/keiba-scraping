"""ResultPageScraper.get_win_payoff()の単体テスト
...
from scraping.config import WIN_PAYOFF_COLUMNS
...
def test_columns_match(race_id: str) -> None:
    """カラム構成がWIN_PAYOFF_COLUMNSと一致すること"""
...
def test_derby_2025_values() -> None:
    """ダービー2025の単勝払い戻しが正しいこと"""
    scraper = create_scraper_from_fixture("202505021211")
    df = scraper.get_win_payoff()

    assert df["単勝払戻金_1"].iloc[0] == 210
    assert df["単勝馬番_1"].iloc[0] == 13
    assert df["単勝人気_1"].iloc[0] == 1
    assert pd.isna(df["単勝払戻金_2"].iloc[0])
    assert pd.isna(df["単勝馬番_2"].iloc[0])
    assert pd.isna(df["単勝人気_2"].iloc[0])


def test_oaks_2010_dead_heat_first() -> None:
    """オークス2010（1着同着2頭）の単勝払い戻しが正しいこと"""
    scraper = create_scraper_from_fixture("201005030211")
    df = scraper.get_win_payoff()

    # 1頭目
    assert df["単勝払戻金_1"].iloc[0] == 210
    assert df["単勝馬番_1"].iloc[0] == 17
    assert df["単勝人気_1"].iloc[0] == 1
    # 2頭目（同着）
    assert df["単勝払戻金_2"].iloc[0] == 380
    assert df["単勝馬番_2"].iloc[0] == 18
    assert df["単勝人気_2"].iloc[0] == 5
    # 3頭目は無し
    assert pd.isna(df["単勝払戻金_3"].iloc[0])


def test_three_way_dead_heat_third() -> None:
    """3着同着3頭レースの単勝払い戻しが正しいこと（同着は3着のみで単勝は1頭）"""
    scraper = create_scraper_from_fixture("202009050712")
    df = scraper.get_win_payoff()

    assert df["単勝払戻金_1"].iloc[0] == 390
    assert df["単勝馬番_1"].iloc[0] == 8
    assert df["単勝人気_1"].iloc[0] == 1
    assert pd.isna(df["単勝払戻金_2"].iloc[0])
